...
for f in fileList:
    if ".txt" in f:
        midList.append(f)
print("Press letter of song to play OR type the name of the song to search")
for i in range(len(midList)):
    print(chr(97 + i), ":", midList[i][:-4])
...
if len(choice) > 2:
    ...
    for item in midList:
        if choice.lower() in item.lower()[:-4]:
            results.append(item)
    print("Press letter of song to play")
    for i in range(len(results)):
        print(chr(97 + i), ":", results[i][:-4])
    choice = input()
    file = open('./songs/' + results[ord(choice) - 97], "r")
else:
    file = open('./songs/' + midList[ord(choice) - 97], "r")
